...
I<lemonbar> [-h | -g I<width>B<x>I<height>B<+>I<x>B<+>I<y> | -r | -b | -d | -f I<font> | -p | -n I<name> | -u I<pixel> | -B I<color> | -F I<color> | -U I<color> | -o I<offset> ]
...
Set next output to I<name>. May be used multiple times; order is significant. If any B<-r> options are given, only B<-r> specified monitors will be used. Invalid output names are silently ignored. (only supported on randr configurations at this time)
...
font.  This version supports fontconfig font specifiers and anti-aliased fonts.
